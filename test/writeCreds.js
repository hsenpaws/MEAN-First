//Create Users StormPath users for authentication
// Needs to be created dynamically as strompath API key return value is unknown.

TU_EMAIL_REGEX = new RegExp('^testuser*');
SP_APP_NAME = 'Reader Test';
TEST_CREDS_TEMP_FILE = './readerTestCreds.js';
...
function writeCreds(callback) {
  var fs = require('fs');
  fs.writeFileSync(TEST_CREDS_TEMP_FILE, 'TEST_USERS = ');
  fs.appendFileSync(TEST_CREDS_TEMP_FILE, JSON.stringify(users_array));
  fs.appendFileSync(TEST_CREDS_TEMP_FILE, '; module.exports = TEST_USERS;');
  callback(0);
